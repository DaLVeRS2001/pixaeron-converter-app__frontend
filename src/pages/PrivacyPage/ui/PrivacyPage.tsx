...
const PrivacyPage = () => (
  <>
    <LegalPageLayout title="Privacy Policy" updated="July 26, 2026">
      <p>
        This policy explains how Pixaeron handles personal data when you create an account,
        sign in, secure your session, or use account recovery. The image-processing product is
        still under development; this policy must be updated before additional data flows are
        launched.
      </p>
      ...
      <ul>
        ...
        <li>
          ...
          email-verification signal, and display name. The Google ID token is sent to our
          backend for verification and is not stored as account data.
        </li>
        <li>
          Security data: session identifiers, a User-Agent value truncated to 512 characters,
          hashed IP-derived identifiers, timestamps, and security/audit events. Passwords,
          reset tokens, verification tokens, CAPTCHA tokens, and Google ID tokens are not
      ...
      <p>
        ...
        for authentication, account creation, account security, and account management.
        Pixaeron does not request Google API access tokens or access Gmail, Google Drive,
        Google Contacts, or other Google services. We do not sell Google user data or use it
        for advertising. Google account data is shared only with service providers when
        necessary to operate and secure Pixaeron, as described below.
      </p>
      <h2>Why we use data</h2>
      <p>
        We use this data to provide accounts, authenticate users, protect the service from
        abuse, maintain sessions, deliver verification and password-reset messages, investigate
        security events, and meet operational obligations.
      </p>
      <h2>Service providers</h2>
      <p>
        Cloudflare provides edge security and Turnstile CAPTCHA; Google provides Google
        Sign-In; Amazon Web Services provides application infrastructure and transactional
        email. Each provider processes limited data required for its role under its own terms
        and privacy commitments.
      </p>
      <h2>Cookies and local storage</h2>
      <p>
        Authentication uses secure HTTP-only cookies. Language preference and the email pending
        verification may be stored locally in your browser. We do not store passwords or
        identity tokens in browser storage.
      </p>
